...
local function sendBufferedLogsToDiscord()
    ...
    PerformHttpRequest(BridgeServerConfig.WebhookURL, function(err, text, headers) end, 'POST', json.encode({
        username = 'brnx_bridge Logger',
        avatar_url =
        'https://avatars.githubusercontent.com/u/192999457?s=400&u=da632e8f64c85def390cfd1a73c3b664d6882b38&v=4',
        embeds = embeds
...
local logHandlers = {
    fivemerr = function(message)
        ...
        PerformHttpRequest('https://api.fivemerr.com/v1/logs', function(code, text, headers)
            if code ~= 200 then
                print(('Failed to send log to Fivemerr API (HTTP %d): Please verify your API key is correct and the service is available.')
                :format(code))
            end
    ...
    fivemanage = function(message)
        if GetResourceState("fmsdk") == "missing" then return print(
            '^1Error:^0 fivemanage log system selected but fmsdk resource is missing.') end
