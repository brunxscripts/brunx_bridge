...
function Marker.Create(data)
    ...
    TriggerClientEvent("brnx_bridge:Client:Marker", -1, Created[_id])
    return _id
end

function Marker.Remove(id)
    if not Marker.Destroy(id) then return end
    TriggerClientEvent("brnx_bridge:Client:MarkerRemove", -1, id)
end
...
function Marker.CreateBulk(datas)
    ...
    TriggerClientEvent("brnx_bridge:Client:MarkerBulk", -1, toClient)
    return toClient
end
...
function Marker.RemoveBulk(ids)
    ...
    TriggerClientEvent("brnx_bridge:Client:MarkerRemoveBulk", -1, toClient)
end
