...
local function SpawnEntity(entityData)
    ...
    if entityData.model then
        ...
        local rotation = entityData.rotation or
        vector3(0.0, 0.0, entityData.heading or 0.0)                                         -- Default rotation if not provided
        ...
        elseif entityData.entityType == 'ped' then
            entity = CreatePed(4, model, coords.x, coords.y, coords.z,
                type(rotation) == 'number' and rotation or rotation.z, false, false)
        elseif entityData.entityType == 'vehicle' then
            entity = CreateVehicle(model, coords.x, coords.y, coords.z,
                type(rotation) == 'number' and rotation or rotation.z, false, false)
        else
            print(string.format("[ClientEntity] Unknown entity type '%s' for entity %s", entityData.entityType,
                entityData.id))
        end
...
function ClientEntity.Create(entityData)
    ...
    entityData.invoked = entityData.invoked or GetInvokingResource() or "brnx_bridge"
    ...
    local entityPoint = Point.Register(entityData.id, entityData.coords, entityData.spawnDistance or 50.0, entityData,
        SpawnEntity, RemoveEntity, UpdateEntity)
...
function ClientEntity.SetAnim(id, anim)
    ...
    Bridge.Anim.Play(id, entity, anim.dict, anim.name, anim.blendIn or 8.0, anim.blendOut or -8.0, anim.duration,
        anim.flags, anim.playbackRate or 0.0, anim.onComplete)
end
...
function ClientEntity.SetAttach(id, attachData)
    ...
    AttachEntityToEntity(entity, targetEntity, GetPedBoneIndex(targetEntity, bone), offset.x, offset.y, offset.z,
...
RegisterNetEvent("brnx_bridge:client:CreateEntity", function(entityData)
    local src = source
    if src ~= 65535 then return end -- Server only
    ClientEntity.Create(entityData)
end)

RegisterNetEvent("brnx_bridge:client:CreateEntities", function(entities)
    local src = source
    if src ~= 65535 then return end -- Server only
    ClientEntity.CreateBulk(entities)
end)

RegisterNetEvent("brnx_bridge:client:DeleteEntity", function(id)
    local src = source
    if src ~= 65535 then return end -- Server only
    ClientEntity.Destroy(id)
end)

RegisterNetEvent("brnx_bridge:client:UpdateEntity", function(id, data)
    local src = source
    if src ~= 65535 then return end -- Server only
