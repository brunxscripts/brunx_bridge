...
function ServerEntity.New(id, entityType, model, coords, rotation, args)
    ...
    local invoking = GetInvokingResource() or "brnx_bridge"
...
function ServerEntity.Create(data)
    local self = ServerEntity.New(data.id, data.entityType, data.model, data.coords,
        data.rotation or vector3(0.0, 0.0, data.heading or 0.0), data)
    if not self then return nil end
    TriggerClientEvent("brnx_bridge:client:CreateEntity", -1, self)
    return self
end
...
function ServerEntity.CreateBulk(entities)
    ...
    for _, entityData in pairs(entities) do
        ...
        local entity = ServerEntity.New(id, entityData.entityType, entityData.model, entityData.coords,
            entityData.rotation, entityData)
        createdEntities[id] = entity
    end
    TriggerClientEvent("brnx_bridge:client:CreateEntities", -1, createdEntities)
...
function ServerEntity.Destroy(id)
    ...
    TriggerClientEvent("brnx_bridge:client:DeleteEntity", -1, id)
end
...
function ServerEntity.DestroyBulk(entityDatas)
    ...
    TriggerClientEvent("brnx_bridge:client:DeleteBulk", -1, entityDatas)
end
...
function ServerEntity.Set(id, data)
    ...
    TriggerClientEvent("brnx_bridge:client:UpdateEntity", -1, id, data)
    return true
end
...
AddEventHandler('brnx_bridge:Server:OnPlayerLoaded', function(source)
    -- print("Player loaded:", source)
    TriggerClientEvent("brnx_bridge:client:CreateEntities", source, Entities)
end)
