...
Inventory.GetItemCount = function(src, item, metadata)
    if metadata then print(
        "qs-inventory does not support metadata searching for item counts, you will need to get the inventory and parse it manually.") end
...
Inventory.OpenStash = function(src, _type, id)
    ...
    TriggerEvent("inventory:server:OpenInventory", _type, id,
        tbl and { maxweight = tbl.weight or 5000, slots = tbl.slots or 20 })
