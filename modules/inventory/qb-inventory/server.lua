...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName ~= "brnx_bridge" then return end
...
Inventory.AddItem = function(src, item, count, slot, metadata)
    ...
    local success = qbInventory:AddItem(src, item, count, slot, metadata, 'brnx_bridge')
...
Inventory.RemoveItem = function(src, item, count, slot, metadata)
    local success = qbInventory:RemoveItem(src, item, count, slot, 'brnx_bridge')
    if not success then return false end
...
Inventory.AddStashItems = function(id, items)
    ...
    for _, item in pairs(items) do
        success = qbInventory:AddItem(id, item.item, item.count or item.amount, nil, item.metadata or item.info,
            "brnx_bridge, adding items to stash")
    end
    return success
end
...
Inventory.AddTrunkItems = function(identifier, items)
    ...
    Wait(1000)
    for i = 1, #items do
        qbInventory:AddItem(fullTrunkId, items[i].name, items[i].amount or items[i].count, items[i].slot or nil,
            items[i].info or items[i].metadata or {}, "brnx_bridge, adding items to trunk")
    end
    return true
end
...
Inventory.UpdatePlate = function(oldplate, newplate)
    ...
    qbInventory:CreateInventory('glovebox-' .. newplate,
        { label = 'glovebox-' .. newplate, slots = storedGloveBox.slots, maxweight = storedGloveBox.maxweight })
    ...
    qbInventory:CreateInventory('trunk-' .. newplate,
        { label = 'trunk-' .. newplate, slots = storedTrunk.slots, maxweight = storedTrunk.maxweight })
...
Inventory.Old.RegisterShop = function(shopTitle, inventory, shopCoords, shopGroups)
    ...
    for _, v in pairs(inventory) do
        table.insert(shopData.items,
            { name = v.name, price = v.price, amount = v.count or v.amount or 1000, info = v.metadata or v.info or {}, type =
            'item' })
    end
    ...
    print(
    "QB-INVENTORY: You are using an outdated version of qb-inventory, please update to the latest version. Stuff will still work but you are using litterally the most exploitable inventory in fivem.")
end
...
Inventory.Old.OpenStash = function(src, _type, id)
    ...
    TriggerClientEvent('brnx_bridge:client:qb-inventory:openStash', src, id,
        { weight = tbl.weight or 5000, slots = tbl.slots or 20 })
end
