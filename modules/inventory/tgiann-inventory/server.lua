...
Inventory.GetPlayerInventory = function(src)
    ...
    for _, v in pairs(inventory) do
        if tonumber(_) then
            table.insert(items,
                { name = v.name, label = v.name, weight = 0, description = v.description, slot = v.slot, count = v
                .amount, metadata = v.info })
        end
    end
    return items
end
...
Inventory.OpenStash = function(src, _type, id)
    ...
    return tgiann:ForceOpenInventory(src, _type, id,
        tbl and { maxWeight = tbl.weight, slots = tbl.slot, label = tbl.label })
end
...
Inventory.GetImagePath = function(item)
    ...
    local imagePath = pngPath and string.format("nui://inventory_images/images/%s.png", pngItem) or
    webpPath and string.format("nui://inventory_images/images/%s.webp", webpItem)
