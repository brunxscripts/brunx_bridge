...
Framework.ItemList = function()
    ...
    local items = Callback.Trigger('brnx_bridge:Callback:GetFrameworkItems', false)
...
Framework.GetFrameworkJobs = function()
    local jobs = Callback.Trigger('brnx_bridge:Callback:GetFrameworkJobs', false)
    return jobs
end
...
Framework.GetItemInfo = function(item)
    return {},
        print(
        "ESX has not implemented GetItemInfo for this framework. Please ensure the inventory you are using is supported and start order is correct.")
end
...
RegisterNetEvent('esx:setJob', function(data)
    TriggerEvent('brnx_bridge:Client:OnPlayerJobUpdate', data.name, data.label, data.grade_label, data.grade)
end)
