...
Framework.GetItem = function(src, item, metadata)
    print(
    "ESX does not support item metadata searches, please ensure you are using a supported inventory system or that you have the correct start order.")
    local xPlayer = Framework.GetPlayer(src)
    if not xPlayer then return {} end
...
Framework.GetItemInfo = function(item)
    ...
    return { name = item, label = items[item].label, stack = false, weight = items[item].weight, description = items
    [item].description, image = items[item].image }
end
...
Framework.GetPlayerInventory = function(src)
    print(
    "ESX does not support item metadata or slots, please ensure you are using a supported inventory system or that you have the correct start order.")
    local xPlayer = Framework.GetPlayer(src)
    if not xPlayer then return {} end
    local playerItems = xPlayer.getInventory()
    local repackedTable = {}
...
Framework.GetItemBySlot = function(src, slot)
    print(
    "ESX does not support item slots, please ensure you are using a supported inventory system or that you have the correct start order.")
...
Framework.GetPlayerGang = function(src)
    print(
    "ESX does not support gangs, please ensure you are using a supported gang system or that you have the correct start order.")
    return
end
...
Framework.AddItem = function(src, item, count, slot, metadata)
    if slot then print(
        "ESX does not support item slots, please ensure you are using a supported inventory system or that you have the correct start order.") end
    if metadata then print(
        "ESX does not support item metadata, please ensure you are using a supported inventory system or that you have the correct start order.") end
    local xPlayer = Framework.GetPlayer(src)
    if not xPlayer then return false end
...
Framework.RemoveItem = function(src, item, amount, slot, metadata)
    ...
    if metadata then
        print(
        "ESX does not support item metadata, please ensure you are using a supported inventory system or that you have the correct start order.")
    end
    local xPlayer = Framework.GetPlayer(src)
    if not xPlayer then return false end
...
Framework.IsVehicleOwnedByPlayer = function(src, plate)
    local citizenId = Framework.GetPlayerIdentifier(src)
    local result = MySQL.Sync.fetchAll("SELECT id, vehicle, plate FROM owned_vehicles WHERE owner = '" ..
    citizenId .. "' AND plate = '" .. plate .. "'")
...
RegisterNetEvent("esx:playerLoaded", function(src)
    src = src or source
    TriggerEvent("brnx_bridge:Server:OnPlayerLoaded", src)
...
RegisterNetEvent("esx:playerLogout", function(src)
    src = src or source
    TriggerEvent("brnx_bridge:Server:OnPlayerUnload", src)
end)
...
RegisterNetEvent("esx:setJob", function(src, job, lastJob)
    ...
    TriggerEvent("brnx_bridge:Server:OnPlayerJobChange", src, job.name)
end)
...
---@param source number
Callback.Register('brnx_bridge:Callback:GetFrameworkJobs', function(source)
...
---@param source number
Callback.Register('brnx_bridge:Callback:GetFrameworkItems', function(source)
