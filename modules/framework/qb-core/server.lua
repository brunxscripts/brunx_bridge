...
Framework.GetItemBySlot = function(src, slot)
    ...
    for _, v in pairs(inventory) do
        if v.slot == slot then
            ...
            return { v.name, itemInfo.label or v.name, itemInfo.weight or 0, v.count or 0, v.metadata or {}, v.slot,
                itemInfo.stack or false, itemInfo.description or "No description available", itemInfo.image or nil }
        end
    end
    return {}
end
...
Framework.IsVehicleOwnedByPlayer = function(src, plate)
    local citizenId = Framework.GetPlayerIdentifier(src)
    local result = MySQL.Sync.fetchAll("SELECT id, vehicle, plate FROM player_vehicles WHERE citizenid = '" ..
