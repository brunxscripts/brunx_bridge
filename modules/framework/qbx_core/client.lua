...
Framework.GetItemCount = function(item)
    return 0,
        print(
        "Qbox has not implemented GetItemCount for this framework. Please ensure the inventory you are using is supported and start order is correct.")
end
...
Framework.GetItemInfo = function(item)
    return {},
        print(
        "Qbox has not implemented GetItemInfo for this framework. Please ensure the inventory you are using is supported and start order is correct.")
end
...
Framework.HasItem = function(item, requiredCount)
    return false,
        print(
        "Qbox has not implemented HasItem for this framework, Please ensure the inventory you are using is supported and start order is correct.")
end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    Wait(1500)
    TriggerEvent('brnx_bridge:Client:OnPlayerLoaded')
end)

---@description Event handler for when player is unloaded in QBX Core framework
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    TriggerEvent('brnx_bridge:Client:OnPlayerUnload')
end)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(data)
    TriggerEvent('brnx_bridge:Client:OnPlayerJobUpdate', data.name, data.label, data.grade.name, data.grade.level)
end)
