...
Shops.OpenShop = function(src, shopName)
    if not shopName and not registeredShops[shopName] then return false,
            print("brnx_bridge PLAYERID " .. src .. " attempted to open invalid shop: " .. shopName) end
    TriggerClientEvent('brnx_bridge:Client:openShop', src, 'shop', shopName, registeredShops[shopName])
    return true
end
...
Shops.CreateShop = function(shopName, shopInventory, shopCoords, shopGroups)
    ...
    registeredShops[shopName] = { name = shopName, inventory = shopInventory, shopCoords = shopCoords, groups =
    shopGroups }
    return true
end
...
Shops.CompleteCheckout = function(src, shopName, item, amount, account)
    ...
    if not itemData or not itemData.price then return print("brnx_bridge Player ID " ..
        src .. " attempted to purchase invalid item from shop: " .. shopName) end
    if itemData.count and itemData.count <= 0 then return Notify.SendNotify(src, locale('Shops.NotEnoughStock'), "error",
            5000) end
...
---@param amount number
---@param account string
RegisterNetEvent("brnx_bridge:Server:completeCheckout", function(shopName, item, amount, account)
    local src = source
