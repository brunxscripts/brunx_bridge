...
function Clothing.SetAppearance(src, data, updateBackup, save)
    ...
    TriggerClientEvent('brnx_bridge:client:SetAppearance', src, Clothing.Players[citId].converted)
    return Clothing.Players[citId]
end
...
AddEventHandler('brnx_bridge:Server:OnPlayerLoaded', function(src)
    src = src and tonumber(src)
    assert(src, "src is nil")
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
Callback.Register('brnx_bridge:cb:GetAppearance', function(source)
    local src = source
