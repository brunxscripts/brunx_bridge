...
function Clothing.OpenMenu()
    print("^6 No supported clothing menu in use in brnx_bridge. ^0 ")
end
...
Callback.Register('brnx_bridge:cb:GetAppearance', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('brnx_bridge:client:SetAppearance', function(data)
    if source ~= 65535 then return end
    Clothing.SetAppearance(PlayerPedId(), data)
end)

RegisterNetEvent('brnx_bridge:client:GetAppearance', function()
    if source ~= 65535 then return end
    Clothing.GetAppearance(PlayerPedId())
end)

RegisterNetEvent('brnx_bridge:client:RestoreAppearance', function()
    if source ~= 65535 then return end
