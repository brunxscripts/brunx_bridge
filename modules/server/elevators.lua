if Config.Elevators then
    ...
    local function isPlayerRestrained(target)
        if Config.ElevatorCanCarryPlayer then
            local ok, allowed = pcall(function()
                return Config.ElevatorCanCarryPlayer(target)
            end)
        ...
        if Config.ElevatorCarryCuffedOnly == false then
            return true
        end

        local state = Player(target).state

        local keys = Config.ElevatorCuffStateKeys or {
            'isCuffed',
            'cuffed',
            'handcuffed',
            'isHandcuffed',
    ...
    local function getClosestCarryTarget(source)
        if Config.ElevatorCarryEnabled == false then return nil end

        local sourcePed = GetPlayerPed(source)
        if not sourcePed or sourcePed == 0 then return nil end

        local sourceCoords = GetEntityCoords(sourcePed)
        local maxDistance = tonumber(Config.ElevatorCarryDistance or 3.0) or 3.0
        ...
        for _, playerId in ipairs(GetPlayers()) do
            ...
            if target and target ~= source and isPlayerRestrained(target) then
        ...
        if closestPlayer then
            debugPrint(('carry target found src=%s target=%s dist=%.2f'):format(
                tostring(source),
                tostring(closestPlayer),
                closestDistance
            ))
        end
    ...
    local function sendTeleport(playerId, floor)
        TriggerClientEvent('brnx_elevator_bridge:client:teleport', playerId, {
            x = floor.coords.x,
            y = floor.coords.y,
            z = floor.coords.z,
            w = floor.coords.w
    ...
    RegisterNetEvent('brnx_elevator_bridge:server:openMenu', function(elevIndex, currentFloorIndex)
        ...
        debugPrint(('openMenu requested src=%s elevator=%s floor=%s'):format(
            tostring(src),
            tostring(elevIndex),
            tostring(currentFloorIndex)
        ))

        if not elevIndex or not currentFloorIndex then
            debugPrint('openMenu cancelled: invalid index')
            return
        end

        if not ActiveElevators[elevIndex] then
            debugPrint('openMenu cancelled: elevator not found')
            return
        end

        local data = buildFloorsForPlayer(src, elevIndex)
        if not data then
            debugPrint('openMenu cancelled: buildFloorsForPlayer returned nil')
            return
        end

        debugPrint(('openMenu sending src=%s name=%s floors=%s'):format(
            tostring(src),
            tostring(data.name),
            tostring(data.floors and #data.floors or 0)
        ))
    ...
    RegisterNetEvent('brnx_elevator_bridge:server:requestTeleport', function(elevIndex, floorIndex)
        ...
        TriggerClientEvent(
            'brnx_elevator_bridge:client:startTeleport',
            src,
            token,
            Config.ElevatorProgressDuration or 5000,
            Config.ElevatorProgressLabel or localeText('elevatorLoading', 'Lift wordt gebruikt...')
        )
    end)

    RegisterNetEvent('brnx_elevator_bridge:server:completeTeleport', function(token, success)
        local src = source

        if type(token) ~= 'string' then return end

        local pending = PendingTeleports[token]
        PendingTeleports[token] = nil

        if not pending or pending.source ~= src then
            return
        end

        if pending.expiresAt and os.time() > pending.expiresAt then
            notify(src, localeText('teleportExpired', 'Lift actie verlopen.'), 'error')
            return
        end

        if success ~= true then
            notify(src, localeText('teleportCancelled', 'Lift geannuleerd.'), 'error')
            return
        end

        local elevator = ActiveElevators[pending.elevIndex]
        if not elevator then return end

        local floor = elevator.floors and elevator.floors[pending.floorIndex]
        if not floor or not floor.coords then return end
        ...
        local carryTarget = getClosestCarryTarget(src)

        sendTeleport(src, floor)

        if carryTarget then
            sendTeleport(carryTarget, floor)
            notify(src, localeText('playerTakenWithElevator', 'Je hebt de speler meegenomen in de lift.'), 'success')
            notify(carryTarget, localeText('takenWithElevator', 'Je bent meegenomen in de lift.'), 'inform')
        end
    end)
end
