...
local function printUpdateAvailable(resource, localVersion, downloadUrl)
    print(('^1An update is available for %s (current version: %s)\r\n - ^5Please download the latest version from %s^7')
    :format(resource, localVersion, downloadUrl))
end
...
local function validateRepoPath(repoPath)
    local username, reponame = repoPath:match("([^/]+)/([^/]+)")
    if not username or not reponame then return false,
            print('Invalid repository format. Expected format: username/reponame') end
...
local function getLocalVersion(resource)
    ...
    if not localVersionRaw then
        return nil,
            print('No version metadata found for resource "%s". Ensure fxmanifest.lua contains a valid version string.')
            :format(resource)
    end
    local version = parseVersion(localVersionRaw)
    if not version then return nil,
            print('Invalid version format for resource "%s". Expected format: "x.x.x"'):format(resource) end
    return version
end
...
--- Example: Version.VersionChecker("TheOrderFivem/brnx_bridge", false)
...
function Version.VersionChecker(repoPath, tebex)
    ...
    if not username or not reponame or not resource then return print(
        '^1Invalid repository format. Expected format: "username/reponame"^0') end
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    -- Version.VersionChecker("TheOrderFivem/brnx_bridge", false)
